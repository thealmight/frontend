...
export default function OperatorDashboard() {
  ...
  useEffect(() => {
    if (!authUser) {
      navigate('/');
    } else if (authUser.role !== 'operator') {
      navigate('/player');
    }
  }, [authUser, navigate]);

  // 🔄 Load fresh game data on mount
  useEffect(() => {
    if (gameId) {
      loadGameData();
    }
  ...
  // 🔁 Reload game data on round change
  useEffect(() => {
    if (gameId) {
      loadGameData();
    }
  ...
  useEffect(() => {
    ...
    socket.emit('operator-join', { gameId });
  ...
  const loadTariffMatrix = async (product) => {
    ...
    try {
      ...
      console.error('Tariff matrix error:', err);
    }
  };
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-lg border-2 ${
                    player ? 'border-green-500 bg-green-50' : 'border-gray-300 bg-gray-50'
                  }`}
                >
